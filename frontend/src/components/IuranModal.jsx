...
import { iuranSchema, STATUS_BAYAR_OPTIONS, TIPE3_OPTIONS, BULAN_OPTIONS, DEFAULT_IURAN_FORM_VALUES, getDefaultAmountForTipe3 } from '../validation/pembayaranSchema';
...
export default function IuranModal({ isOpen, onClose, onSubmit, iuran, mode, rumahOptions }) {
    ...
    const {
        ...
        resolver: yupResolver(iuranSchema),
        defaultValues: DEFAULT_IURAN_FORM_VALUES,
    });
    ...
    useEffect(() => {
        if (tipe3Value && !isEdit) {
            const defaultAmount = getDefaultAmountForTipe3(tipe3Value);
            setValue('bayar', defaultAmount);
        }
    }, [tipe3Value, setValue, isEdit]);
    ...
    useEffect(() => {
        if (isOpen) {
            if (isEdit && iuran) {
                reset({
                    id_data: iuran.id_data || '',
                    tipe3: iuran.tipe3 || 'kebersihan',
                    bulan: iuran.bulan || new Date().getMonth() + 1,
                    tahun: iuran.tahun || new Date().getFullYear(),
                    bayar: iuran.bayar || getDefaultAmountForTipe3(iuran.tipe3),
                    status_bayar: iuran.status_bayar || 'lunas',
                    tanggal_bayar: iuran.tanggal_bayar || new Date().toISOString().split('T')[0],
                });
            } else {
                reset(DEFAULT_IURAN_FORM_VALUES);
            }
        }
    }, [isOpen, isEdit, iuran, reset]);
    ...
    const onFormSubmit = (data) => {
        ...
        onSubmit(submitData, isEdit ? iuran?.id : null);
    };
                ...
                <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            disabled={isSubmitting}
                            className={`w-full px-3 py-2 rounded-lg border bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all disabled:opacity-50 disabled:cursor-not-allowed ${errors.bulan
                                ? 'border-red-500 dark:border-red-500'
                                : 'border-gray-200 dark:border-gray-700'
                                }`}
                        >
